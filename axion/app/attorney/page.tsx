...
export default function AttorneyPage() {
  ...
  const [transferCardView, setTransferCardView] = useState(false)
  ...
  useEffect(() => {
    async function load() {
      ...
      setRequests(reqs ?? [])
      if (prof) {
        setTransferContact({ attorney_name: prof.attorney_name || '', attorney_firm: prof.attorney_firm || '', attorney_email: prof.attorney_email || '', attorney_phone: prof.attorney_phone || '', attorney_notes: prof.attorney_notes || '' })
        if (prof.attorney_name || prof.attorney_email) setTransferCardView(true)
      }
      setLoading(false)
    }
    load()
  ...
  async function saveTransferContact(e: React.FormEvent) {
    ...
    // Send notification email to attorney (only on first save or if email changed)
    if (wasNew && transferContact.attorney_email) {
      fetch('/api/notify-attorney', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          attorney_name: transferContact.attorney_name,
          attorney_email: transferContact.attorney_email,
          attorney_firm: transferContact.attorney_firm,
          grantor_name: prof?.full_name || user?.email,
        }),
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          ...
          <div style={{ marginBottom: '32px' }}>
            ...
            {/* Saved card view */}
            {transferCardView ? (
              <div style={{ background: 'rgba(8,14,40,0.85)', border: '1px solid rgba(0,204,102,0.25)', borderRadius: '14px', padding: '18px 20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                    <span style={{ fontSize: '15px', fontWeight: 700, color: '#fff' }}>{transferContact.attorney_name || 'Estate Attorney'}</span>
                    <span style={{ fontSize: '11px', fontWeight: 700, padding: '2px 8px', borderRadius: '6px', background: 'rgba(0,204,102,0.1)', border: '1px solid rgba(0,204,102,0.25)', color: '#00cc66' }}>✓ Configured</span>
                  </div>
                  <div style={{ fontSize: '12px', color: '#6b7ab8', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    {transferContact.attorney_firm && <span>{transferContact.attorney_firm}</span>}
                    {transferContact.attorney_email && <span>· {transferContact.attorney_email}</span>}
                    {transferContact.attorney_phone && <span>· {transferContact.attorney_phone}</span>}
                  </div>
                  {transferContact.attorney_notes && (
                    <div style={{ fontSize: '12px', color: '#4a5578', marginTop: '5px', fontStyle: 'italic' }}>{transferContact.attorney_notes}</div>
                  )}
                </div>
                <button
                  onClick={() => setTransferCardView(false)}
                  style={{ flexShrink: 0, padding: '7px 16px', background: 'transparent', border: '1px solid rgba(0,100,255,0.25)', borderRadius: '8px', color: '#6b7ab8', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>
                  ✏️ Edit
                </button>
              </div>
            ) : (
              ...
              <div style={{ background: 'rgba(102,68,255,0.06)', border: '1px solid rgba(102,68,255,0.2)', borderRadius: '16px', padding: '24px', marginBottom: '16px' }}>
                ...
                {transferSaved && (
                  <div style={{ padding: '10px 14px', background: 'rgba(0,204,102,0.08)', border: '1px solid rgba(0,204,102,0.25)', borderRadius: '8px', color: '#00cc66', fontSize: '13px', fontWeight: 600, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span>✓</span> Saved — attorney notified by email
                  </div>
                )}
                ...
                <form onSubmit={saveTransferContact} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                  ...
                  <div style={{ gridColumn: '1/-1', display: 'flex', justifyContent: 'flex-end' }}>
                    <button type="submit" disabled={transferSaving} style={{ padding: '10px 24px', background: transferSaving ? 'rgba(102,68,255,0.3)' : 'linear-gradient(135deg,#6644ff,#aa88ff)', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '13px', fontWeight: 700, cursor: transferSaving ? 'not-allowed' : 'pointer' }}>
