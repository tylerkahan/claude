...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Draft Documents', href: '/draft', badge: null, badgeRed: false, icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> },
      ...
      { label: 'Entity Tree', href: '/entities', badge: null, badgeRed: false, icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="5" r="2"/><circle cx="5" cy="19" r="2"/><circle cx="19" cy="19" r="2"/><path d="M12 7v4M12 11l-5.5 6M12 11l5.5 6"/></svg> },
    ]
  },
  {
    label: 'Tools',
    items: [
      ...
      { label: 'Tax Optimizer', href: '/tax', badge: null, badgeRed: false, icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2a10 10 0 100 20A10 10 0 0012 2z"/><path d="M12 6v6l4 2"/></svg> },
      ...
      { label: 'Inheritance Sim', href: '/simulator', badge: null, badgeRed: false, icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> },
  ...
  const sidebarContent = (
    ...
    <div style={{
      ...
      <div style={{ padding: '18px 16px 16px', borderBottom: '1px solid rgba(0,100,255,0.1)', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        {isMobile && (
          <button onClick={() => setOpen(false)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#6b7ab8', cursor: 'pointer', fontSize: '20px', lineHeight: 1, padding: '2px 6px' }}>×</button>
        )}
      </div>
    ...
    return (
      <>
        ...
        <button
          ...
          style={{ position: 'fixed', top: '13px', left: '14px', zIndex: 200, background: 'rgba(6,10,32,0.95)', border: '1px solid rgba(0,100,255,0.2)', borderRadius: '8px', padding: '7px 9px', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '4px' }}
        >
          <span style={{ width: '18px', height: '2px', background: '#6b7ab8', borderRadius: '2px', display: 'block' }} />
          <span style={{ width: '18px', height: '2px', background: '#6b7ab8', borderRadius: '2px', display: 'block' }} />
          <span style={{ width: '18px', height: '2px', background: '#6b7ab8', borderRadius: '2px', display: 'block' }} />
        </button>
